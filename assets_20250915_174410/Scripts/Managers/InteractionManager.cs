...
namespace ProjectWitchcraft.Managers
{
    public class InteractionManager : Singleton<InteractionManager>
    {
        ...
        private void HandleTargeting()
        {
            ...
            {
                // NEW: After finding an object with the raycast, we now check the distance from the PLAYER.
                if (Vector3.Distance(_playerTransform.position, hit.transform.position) <= _interactionDistance)
                {
